        ...
        private void GetSanpham(int Comp_ID)
        {
            try
            {
                ...
                using (SqlCommand myCmd = new SqlCommand("dbo.usp_GetSanpham", myCon))
                {
                    ...
                    myCmd.Parameters.Add("@IDSP", SqlDbType.Int).Value = Comp_ID;
                    ...
                    if (myDr.HasRows)
                    {
                        while (myDr.Read())
                        {
                            txtSanphamName.Text = myDr.GetValue(2).ToString();
                            txtSanphamMota.Text = myDr.GetValue(3).ToString();
                            //txtSanphamMa.Text = myDr.GetValue(2).ToString();
                            //txtSanphamDVT.Text = myDr.GetValue(4).ToString();
                            txtDongia.Text = myDr.GetValue(4).ToString();
                            txtSoluong.Text = "1";
                            lblSPID.Text = Comp_ID.ToString();
                            Image1.ImageUrl = "~/IMAGE/" + myDr.GetValue(1).ToString();
                        }
                    }
                }
            }
